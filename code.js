...
figma.ui.onmessage = (message) => __awaiter(this, void 0, void 0, function* () {
    if (message.type === "preview") {
        try {
            ...
            const items = yield buildImagePreview(options);
    ...
    try {
        ...
        yield waitForUiPaint();
        const result = yield exportCurrentSelection(options, progress);
...
function exportCurrentSelection(options, progress) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const canvasBounds = unionBounds(roots.map((node) => node.absoluteBoundingBox));
        const context = {
            assets: [],
            warnings: [],
            usedPaths: new Set(),
        ...
        for (const root of roots) {
            const parsed = yield parseNode(root, canvasBounds, options, context, true);
            if (parsed) {
                nodes.push(parsed);
            }
        }
...
function updateExportProgress(progress, stage, percent) {
...
function countNodeExportImages(node, options) {
    ...
    const shouldRasterize = Object.keys(effects).some((key) => effects[key] === "rasterize");
...
function buildImagePreview(options) {
    return __awaiter(this, void 0, void 0, function* () {
        const roots = figma.currentPage.selection.filter((node) => canReadBounds(node));
        const items = [];
        ...
        for (const root of roots) {
            ...
            yield collectPreviewItems(root, options, items, usedPaths);
        }
        return items;
    });
}
function collectPreviewItems(node, options, items, usedPaths) {
    return __awaiter(this, void 0, void 0, function* () {
        if (items.length >= MAX_PREVIEW_ITEMS) {
            return;
        }
        ...
        if (shouldExportBackground) {
            items.push(yield exportNodeBackgroundPreview(node, bounds, usedPaths));
            if (items.length >= MAX_PREVIEW_ITEMS) {
                return;
            }
        }
        if (type === "sprite" || type === "image") {
            items.push(yield exportNodePreview(node, bounds, usedPaths));
            return;
        }
        if (hasChildren && type === "container" && "children" in node) {
            for (const child of node.children) {
                yield collectPreviewItems(child, options, items, usedPaths);
            }
        }
    });
}
function parseNode(node, parentBounds, options, context, isRoot) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!options.includeHiddenLayers && node.visible === false) {
            return null;
        }
        ...
        if (Object.keys(effects).length > 0) {
            ...
            for (const key in effects) {
                context.warnings.push({
                    nodeId: node.id,
                    nodeName: node.name,
                    message: `${formatEffectName(key)}已${formatEffectPolicy(effects[key])}。`,
                });
            }
        }
        if (type === "label" && node.type === "TEXT") {
            ...
            const asset = yield exportNodeImage(node, bounds, options, context.usedPaths);
        ...
        if (shouldExportBackground) {
            const asset = yield exportNodeBackgroundImage(node, bounds, options, context.usedPaths);
        ...
        if (hasChildren && type === "container" && "children" in node) {
            const children = [];
            for (const child of node.children) {
                const parsed = yield parseNode(child, bounds, options, context, false);
                if (parsed) {
...
function classifyNode(node, hasChildren, shouldRasterize) {
...
function toCocosPosition(bounds, parentBounds) {
...
function parseTextNode(node, options) {
...
function mapCocosHorizontalAlign(value) {
    if (value === "CENTER") {
        return "CENTER";
    }
    if (value === "RIGHT") {
...
function mapCocosVerticalAlign(value) {
    if (value === "CENTER") {
        return "CENTER";
    }
    if (value === "BOTTOM") {
...
function extractTextShadow(node) {
    ...
    const shadow = node.effects.find((e) => e.visible !== false && (e.type === "DROP_SHADOW" || e.type === "INNER_SHADOW"));
...
function getEffectDecision(node, options) {
    const result = {};
...
function getClipsContent(node) {
    return "clipsContent" in node && node.clipsContent === true;
}
function shouldRasterizeOwnAppearance(node, effects) {
    if (!isFrameLikeNode(node)) {
        return false;
    }
    return hasVisibleFill(node) || hasVisibleStroke(node) || Object.keys(effects).some((key) => effects[key] === "rasterize");
}
...
function exportNodeImage(node, bounds, options, usedPaths) {
    return __awaiter(this, void 0, void 0, function* () {
        const maybeExportable = node;
        ...
        const bytes = yield maybeExportable.exportAsync({
            format: "PNG",
            constraint,
        });
...
function exportNodeBackgroundImage(node, bounds, options, usedPaths) {
    return __awaiter(this, void 0, void 0, function* () {
        const constraint = getSafeExportConstraint(bounds, options.imageScale);
        const bytes = yield exportNodeOwnAppearance(node, constraint);
...
function exportNodeBackgroundPreview(node, bounds, usedPaths) {
    return __awaiter(this, void 0, void 0, function* () {
        const longestSide = Math.max(bounds.width, bounds.height);
        const constraint = bounds.width >= bounds.height
            ? { type: "WIDTH", value: Math.max(1, Math.min(80, Math.round(bounds.width))) }
            : { type: "HEIGHT", value: Math.max(1, Math.min(80, Math.round(bounds.height))) };
        const bytes = yield exportNodeOwnAppearance(node, longestSide > 0 ? constraint : { type: "SCALE", value: 1 });
        const path = makeUniquePath(`images/${sanitizeName(node.name)}_background.png`, usedPaths);
        return {
...
function exportNodeOwnAppearance(node, constraint) {
    return __awaiter(this, void 0, void 0, function* () {
        const maybeCloneable = node;
        if (typeof maybeCloneable.clone !== "function") {
            throw new Error(`节点“${node.name}”无法克隆用于背景导出。`);
        }
        const clone = maybeCloneable.clone();
        try {
            ...
            return yield clone.exportAsync({
                format: "PNG",
                constraint,
            });
        }
...
function exportNodePreview(node, bounds, usedPaths) {
    return __awaiter(this, void 0, void 0, function* () {
        const maybeExportable = node;
        if (typeof maybeExportable.exportAsync !== "function") {
...
function getSafeExportConstraint(bounds, requestedScale) {
...
function getConstrainedSize(bounds, constraint) {
...
function formatEffectPolicy(value) {
